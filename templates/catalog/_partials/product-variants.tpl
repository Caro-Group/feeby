            ...
            {foreach from=$group.attributes key=id_attribute item=group_attribute}
              <li class="mb-2 w-full phone:w-1/2 tablet:w-auto">
                <label class="mb-0 mr-2">
                  ...
                  <span class="bg-gray-default block border-2 border-solid cursor-pointer p-3 peer-checked:border-main peer-checked:text-gray-main rounded text-[#BABABA] transition text-sm tablet:text-base">{$group_attribute.name}</span>
                </label>
              </li>
            {/foreach}
          </ul>
