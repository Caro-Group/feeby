  ...
  {foreach from=$groups key=id_attribute_group item=group}
    {if !empty($group.attributes)}
      ...
      <div class="{if $group.group_name|strstr:"Rodzaj fototapety"}hidden{/if} clearfix product-variants-item flex flex-col items-start mb-5 {if $id_attribute_group|in_array:$ids_with_container} mt-[30px] py-5 px-2.5 tablet:p-[30px] border-2 border-gray-2000 border-solid rounded-[5px] {/if}">
        <span class="block text-base text-gray-main text-base tablet:text-sm leading-normal mb-2 font-medium">
        ...
        {if $group.group_type == 'select'}
          <div class="flex gap-2.5 flex-wrap tablet-wide:flex-nowrap">
            <select
              class="form-select taxt-base py-2.5 pl-[30px] bg-[center_right_30px] bg-gray-default rounded border border-gray-2000 hover:border-gray-3000 !ring-0 !outline-0 !outline-offset-0 focus:!border-gray-3000 w-full focus:!ring-0 transition"
            ...
            {if $group.group_name == "Rozmiar" || $id_attribute_group == 6}
              ...
              <a href="/" target="_blank" class="min-w-[152px] group flex justify-center items-center w-full shrink-0 px-5 py-[5px] rounded-[5px] border border-solid border-gray-2000 hover:border-gray-3000 transition duration-300">
                <svg class="mr-5" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                ...
                <span class="font-medium text-main-dark text-sm tablet:text-xs group-hover:!text-main transition duration-300">
                  {l s='Try out our generator' d='Shop.Theme.Actions'}
                </span>
              </a>
            ...
            {foreach from=$group.attributes key=id_attribute item=group_attribute}
              <li class="shrink basis-0 tablet:basis-1/5 pb-2 pr-2">
                <label class="mb-0 w-full h-full">
                  ...
                  <span class="bg-gray-default flex justify-center items-center text-center h-full border-2 border-solid cursor-pointer px-2 py-4 hover:border-gray-3000 peer-checked:border-main text-gray-main rounded transition text-sm !leading-[21px] min-w-[120px] word-break">{$group_attribute.name}</span>
                </label>
              </li>
            {/foreach}
          </ul>
      ...
      <hr class="my-[20px] {if $group.group_name == "Typ" && $group.attributes['416'] && !$group.attributes['416'].selected || $group@last}desktop:hidden{/if}">
    {/if}
  {/foreach}
</div>