    ...
    {if !empty($group.attributes)}
      ...
      <div class="clearfix product-variants-item flex flex-col items-start mb-5">
        ...
        {elseif $group.group_type == 'color'}
          ...
          <ul id="group_{$id_attribute_group}" class="flex w-full flex-wrap justify-center">
            {foreach from=$group.attributes key=id_attribute item=group_attribute}
                ...
                <label aria-label="{$group_attribute.name}" class="flex flex-col">
                  ...
                  <span
                    {if $group_attribute.texture}
                      class="color texture w-full h-20 tablet:h-[120px] bg-no-repeat bg-cover rounded-[5px] border-2 border-solid border-white hover:border-white peer-checked:border-main opacity-50 peer-checked:opacity-100 transition shadow-none aspect-[4/3]" style="background-image: url({$group_attribute.texture})"
