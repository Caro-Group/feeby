    ...
    {if !empty($group.attributes)}
      ...
      <div class="{if $group.group_name|strstr:"Rodzaj fototapety"}hidden{/if} clearfix product-variants-item flex flex-col items-start mb-5 {if $id_attribute_group eq 23 } mt-10 py-5 px-2.5 tablet:p-[30px] border-2 border-gray-2000 border-solid rounded-[5px] {/if}">
        ...
        {if $group.group_type == 'select'}
          ...
          <select
            class="form-select taxt-base tablet:text-xl py-[18px] tablet:py-4 pl-[30px] bg-[center_right_30px] bg-gray-default rounded border-2 border-gray-2000 hover:border-gray-3000 focus:!border-main w-full mb-[10px] focus:!ring-0 transition"
        ...
        <ul id="group_{$id_attribute_group}" class="flex w-full flex-wrap {if $id_attribute_group neq 23 } justify-center {/if} tablet:justify-start">
            {foreach from=$group.attributes key=id_attribute item=group_attribute}
              <li class="input-container w-full {if $id_attribute_group eq 23 } mx-2.5 tablet:ml-0 tablet:mr-[35px] max-w-[80px] {else} mr-5 phone:max-w-[40%] tablet:max-w-[121px] tablet:aspect-ratio-[4/3] tablet:mr-5 desktop:max-w-[29%] {/if} ">
                ...
                <span class="text-main-dark p-0 text-center text-base {if $id_attribute_group eq 23 } pt-1 tablet-medium:text-lg whitespace-nowrap {else} pt-[10px] tablet-medium:text-2xl {/if}">{$group_attribute.name}</span>
                </label>
              </li>
            {/foreach}
          </ul>
            ...
            {foreach from=$group.attributes key=id_attribute item=group_attribute}
              <li class="flex-1 mb-2">
                <label class="mb-0 mr-2 h-full">
                  ...
                  <span class="bg-gray-default flex items-center text-center h-full border-2 border-solid cursor-pointer p-3 hover:border-gray-3000 peer-checked:border-main text-gray-main rounded transition {if $breadcrumb.links[1].title == 'Parawany'} text-sm tablet:text-base {else} text-base tablet:text-xl {/if}">{$group_attribute.name}</span>
                </label>
              </li>
            {/foreach}
          </ul>
        ...
        <div class="flex flex-wrap leading-normal text-[14px] tablet:text-base text-[#ADAFBA] gap-2.5 tablet:gap-5">
          ...
          {elseif $group.group_name == "Typ"}
            ...
            {if $breadcrumb["count"] > 0 && $breadcrumb["links"][1]["title"]|strstr:"Parawany"}
              {l s='Learn more about the' d='Shop.Theme.Actions'} <a class="underline font-medium text-[#ADAFBA] hover:text-main hover:underline transition duration-200" href="/nasze-parawany" target="_blank">{l s='manufacturing technology' d='Shop.Theme.Actions'}</a>
            {else}
              {l s='Learn more about the' d='Shop.Theme.Actions'} <a class="underline font-medium text-[#ADAFBA] hover:text-main hover:underline transition duration-200" href="/technologia-wykonania" target="_blank">{l s='manufacturing technology' d='Shop.Theme.Actions'}</a>
            {/if}
            </span>
          {/if}
        </div>
