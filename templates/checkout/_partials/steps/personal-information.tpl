  ...
  {if $customer.is_logged && !$customer.is_guest}
    <div class="p-5 tablet:p-0">
      ...
      <form method="GET" action="{$urls.pages.order}">
        <button
          class="bg-main-dark hover:bg-main-hover duration-150 border-0 rounded-full text-white p-2 px-4 uppercase whitespace-nowrap mb-2 flex items-center justify-between cursor-pointer ml-auto"
          name="controller"
          type="submit"
          value="order"
        >
    ...
    <div class=" bg-gray-1000 inset-0 tablet:rounded-md z-10 tablet:pb-12" data-login-panel>
      <div class="flex flex-wrap tablet-wide:flex-nowrap">
        <div class="border-0 border-b border-gray-3000 border-solid flex flex-col items-start mb-0 my-10 pb-10 px-0 mx-5 tablet:px-5 tablet:mx-0 tablet-wide:px-24 tablet:border-b-0 tablet:border-r tablet:mb-8 tablet:my-20 tablet:w-1/2 w-full">
          ...
          {if $guest_allowed}
            <a
              class="bg-main-dark border-0 cursor-pointer duration-150 flex font-body hover:text-white hover:bg-main-hover items-center justify-between max-w-[320px] mb-3 px-5 py-3 rounded-full tablet:max-w-[300px] text-white text-xs  tablet:text-sm uppercase w-full whitespace-nowrap"
