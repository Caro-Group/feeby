...
{block name='step_content'}
  ...
  <div
    class="{if !($customer.addresses|count) || $show_delivery_address_form || $show_invoice_address_form } pointer-events-none opacity-5 {/if} p-5 tablet:p-0">
      ...
      {if $delivery_options|count}
        <form class="clearfix" id="js-delivery"
                        ...
                        {if $carrier.logo}
                          <div class="flex-none tablet-wide:ml-auto">
                            <img src="{$carrier.logo}" alt="{$carrier.name}"
                              class="w-9 h-7 tablet:w-16 tablet:h-12 object-contain rounded-md w-auto" />
                          </div>
                        {/if}
                      </div>
                    </label>
          ...
          <div
            ...
            <button type="submit"
              class="continue w-full text-xs tablet:text-sm tablet:w-auto bg-main hover:bg-main-hover duration-150 border-0 rounded-full text-white p-3 px-4 uppercase whitespace-nowrap mb-8 tablet:mb-0 flex items-center justify-between ml-auto cursor-pointer"
