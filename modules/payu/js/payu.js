...
$(document).ready(function () {
  $('.payMethodEnable .payMethodLabel').on('click', function () {
...
function groupPayuMethod() {
  ...
  for (var i = 0; i < 20; ++i) {
    var isFound =
      $('#payment-option-' + i + '-additional-information .payu-marker-class')
        .length > 0
    if (isFound) {
      payuIndexes.push(i)
    }
  ...
  if (payuIndexes.length > 0) {
    $('.payment-options').append(
      "<fieldset id='payu-methods-grouped' class='payu-payment-fieldset-1-7'>" +
  ...
  for (var indexOfPayuElement in payuIndexes) {
    var element1 = $(
      '#payment-option-' + payuIndexes[indexOfPayuElement] + '-container',
    ).parent()
    var element2 = $(
      '#payment-option-' +
        payuIndexes[indexOfPayuElement] +
        '-additional-information',
    )
    var element3 = $(
      '#pay-with-payment-option-' + payuIndexes[indexOfPayuElement] + '-form',
    )
