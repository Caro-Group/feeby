...
$(document).ready(function () {
  ...
  if ($('.leoquicklogin-slidebar').length) {
    for (var i = 0; i < 3; i++) {
      $('.leoquicklogin-slidebar')
        .first()
        .clone()
        .insertAfter('.leoquicklogin-slidebar:last')
      var slidebar_class = ''
      if (leo_push == 1) {
      ...
      if (!$(this).hasClass('active')) {
        ...
        if (type == 'popup') {
          ...
          if (layout != 'both') {
            ...
            $('.leo-quicklogin-modal .leo-form')
              .removeClass('leo-form-active full-width')
              .addClass('leo-form-inactive full-width')
            $('.leo-quicklogin-modal .leo-' + layout + '-form')
              .removeClass('leo-form-inactive')
              .addClass('leo-form-active full-width')
          } else {
            //DONGND:: inactive tab navigation
            $('.leo-quicklogin-modal .lql-action').hide()

            $('.leo-quicklogin-modal .leo-form')
              .removeClass('leo-form-inactive full-width')
              .addClass('leo-form-active')
          }
        ...
        if (
          ...
          if (layout != 'both') {
            ...
            $('.leoquicklogin-slidebar .lql-bt-' + layout).addClass(
              'lql-active',
            )
            $('.leoquicklogin-slidebar .lql-action').show()

            $('.leoquicklogin-slidebar.' + prefix_class + ' .leo-form')
              .removeClass('leo-form-active full-width')
              .addClass('leo-form-inactive full-width')
            $(
              '.leoquicklogin-slidebar.' +
          ...
          if (leo_push == 1) {
            ...
            if (type == 'slidebar_left' || type == 'slidebar_right') {
              ...
              if (type == 'slidebar_left') {
                push_value = $(
                  '.leoquicklogin-slidebar.' + prefix_class,
                ).outerWidth()
              }
  ...
  $('.leo-resetpass-form-content').on('submit', function () {
    ...
    if (
      ...
      $(this).find('.leo-has-error').length
    ) {
      return false
    }
  })
          ...
          success: function (result) {
            ...
            if (object_result.errors.length) {
              $.each(object_result.errors, function (key, val) {
                parent_obj
                  .find('.lql-form-mesg.has-danger')
                  .append(
                    '<label class="form-control-label">' + val + '</label>',
                  )
              })
              ...
              parent_obj
                .find('.lql-form-mesg.has-success')
                .append(
                  '<label class="form-control-label">' +
                    object_result.success[0] +
                    '<strong>' +
                    lql_email_reset +
                    '</strong></label>',
                )
    ...
    if (!$(this).hasClass('active') && !$(this).hasClass('success')) {
      ...
      parent_obj.find('input').each(function () {
        ...
        if (!$(this).val()) {
          ...
          //check box
          if (
            $(this).hasClass('lql-register-check') &&
            !$(this).is(':checked')
          ) {
            $(this).parent('.form-group').addClass('leo-has-error')
          } else {
      ...
      if (!parent_obj.find('.leo-has-error').length) {
        ...
        var lql_register_firstname = $.trim(
          parent_obj.find('.lql-register-firstname').val(),
        )
        var lql_register_lastname = $.trim(
          parent_obj.find('.lql-register-lastname').val(),
        )
        var lql_register_email = $.trim(
          parent_obj.find('.lql-register-email').val(),
        )
        var lql_register_pass = $.trim(
          parent_obj.find('.lql-register-pass').val(),
        )
        $.ajax({
          type: 'POST',
          headers: { 'cache-control': 'no-cache' },
  ...
  $('.twitter-login-bt').on('click', function () {
    window.open(
      ...
      '_blank',
      'toolbar=yes, scrollbars=yes, resizable=yes, top=100, left=300, width=700, height=600',
    )
  })
})
...
function callLoginForm($element) {
  ...
  if (
    $element.hasClass('lql-callreset-action') &&
    !parent_obj.find('.leoquicklogin-forgotpass').hasClass('active')
  ) {
  ...
  parent_obj
    ...
    .addClass('leo-form-active')
  parent_obj
    .find('.leo-register-form')
    .removeClass('leo-form-active')
    .addClass('leo-form-inactive')
...
function processFbAPI() {
  ...
  setTimeout(function () {
    FB.api(
      '/me?fields=email,birthday,first_name,last_name,name,gender',
      function (response) {
        if (response.email) {
          // console.log(response);
          $('.lql-social-modal .lql-social-modal-mesg').removeClass('active')
          $(
            '.lql-social-modal .lql-social-modal-mesg.lql-social-loading',
          ).addClass('active')
          ...
          $.ajax({
            ...
            success: function (result) {
              $('.lql-social-modal .lql-social-modal-mesg').removeClass(
                'active',
              )
              var object_result = $.parseJSON(result)
              ...
              if (object_result.errors.length) {
                $(
                  '.lql-social-modal .lql-social-modal-mesg.error-login',
                ).addClass('active')
              } else {
                $('.lql-social-modal .lql-social-modal-mesg.success').addClass(
                  'active',
                )
                if (lql_redirect) {
                  window.location.replace(lql_myaccount_url)
          ...
          $('.lql-social-modal .lql-social-modal-mesg').removeClass('active')
          $('.lql-social-modal .lql-social-modal-mesg.error-email').addClass(
            'active',
          )
          $('.lql-social-modal').modal('show')
          // console.log('Fail');
            ...
            $.ajax({
              ...
              success: function (result) {
                ...
                if (object_result.errors.length) {
                  ...
                  $(
                    '.lql-social-modal .lql-social-modal-mesg.success',
                  ).addClass('active')
                  if (lql_redirect) {
                    window.location.replace(lql_myaccount_url)
                  } else {
...
function twitterLogin(first_name, last_name, email) {
  ...
  setTimeout(function () {
    if (email.length) {
      ...
      $('.lql-social-modal .lql-social-modal-mesg').removeClass('active')
      $('.lql-social-modal .lql-social-modal-mesg.lql-social-loading').addClass(
        'active',
      )
      $('.lql-social-modal').modal('show')
      // console.log('Successful login for: ' + response.name);
      ...
      $.ajax({
        ...
        success: function (result) {
          ...
          if (object_result.errors.length) {
            $('.lql-social-modal .lql-social-modal-mesg.error-login').addClass(
              'active',
            )
          } else {
            $('.lql-social-modal .lql-social-modal-mesg.success').addClass(
