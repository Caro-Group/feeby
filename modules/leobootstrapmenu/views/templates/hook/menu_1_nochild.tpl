...
{if $menu.active == 1}
    <li data-menu-type="{$menu.type}" class="nav-item  {$menu.menu_class} {$addwidget}" {$model->renderAttrs($menu)}>
        <a class="nav-link has-category font-body text-main-dark text-base " href="{$model->getLink($menu)}" target="{$menu.target}">
            {if $menu.icon_class}
            ...
            {if $menu.show_title}
                <span class="menu-title font-body text-main-dark text-base font-normal tablet-medium:font-light">{$menu.title}</span>
            {/if}
            {if $menu.text}
