      ...
      <section class="facet clearfix">
        ...
        <div class="title mr-12  {if $_collapse}collapsed{/if}" data-parent="#search_filters" data-target="#facet_{$_expand_id}" data-toggle="collapse"{if !$_collapse} aria-expanded="true"{/if}>
          ...
          <span class="pl-4 -top-1 relative">
            <i class="material-icons text-xl text-main-dark">add</i>
          </span>
        </div>

        {if in_array($facet.widgetType, ['radio', 'checkbox'])}
          {block name='facet_item_other'}
            <ul id="facet_{$_expand_id}" class="collapse max-w-[350px] -ml-[19px] mt-[17px]">
        ...
        {elseif $facet.widgetType == 'dropdown'}
          {block name='facet_item_dropdown'}
            <ul id="facet_{$_expand_id}" class="collapse{if !$_collapse} in{/if} max-w-[350px] -ml-[19px] mt-[17px]">
              <li class="border-0 border-b border-solid border-white px-5 py-2">
          ...
          {block name='facet_item_slider'}
            {foreach from=$facet.filters item="filter"}
              <ul id="facet_{$_expand_id}"
                class="faceted-slider collapse{if !$_collapse} in{/if} max-w-[350px] -ml-[19px] mt-[17px]"
