        ...
        {foreach from=$displayedFacets item="facet"}
          <div class="tablet:pr-5 tablet:py-2.5">
            <section class="facet clearfix">
              ...
              <div
                class="title cursor-pointer px-5 tablet:px-0 border-0 border-b-2 tablet:border-b-0 border-gray-1000 border-solid py-5 tablet:py-0 justify-between tablet:justify-start group {if $_collapse}collapsed{/if}"
                ...
                {block name='facet_item_other'}
                  <ul id="facet_{$_expand_id}"
                    ...
                    {foreach from=$facet.filters key=filter_key item="filter"}
                      ...
                      {if $facet.type == 'attribute_group'}
                        {if $facet.properties.id_attribute_group == '15'}
                          <li class="{if $filter.active} active {/if}">
                            <a href="{$filter.nextEncodedFacetsURL}" class="flex flex-col" rel="nofollow">
                              <img
                                class="object-cover w-44 h-25 rounded-t-md border-2 border-solid transition hover:border-main {if $filter.active} border-main {else} border-transparent {/if}"
                                src="{$filter.properties.texture}" />
                              <h3 class="text-main-dark text-lg font-light font-body ml-5">{$filter.label}</h3>
                            </a>
                          </li>
                        {/if}
                      {/if}
