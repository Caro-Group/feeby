...
function p24DisplayExtrachargeReturn(
  extrachargeText,
  extrachargeReturnFormatted,
  currency,
) {
  if (
    'undefined' !== typeof currency &&
    0 !== $('.order-confirmation-table .font-weight-bold').length
  ) {
...
function p24DisplayExtrachargeAdmin(
  extrachargeText,
  extrachargeReturnFormatted,
  currency,
) {
  if (
    'undefined' !== typeof currency &&
    0 !== $('.panel .panel-total .table #total_order').length
  ) {
    $('.panel .panel-total .table #total_order').before(
      '<tr id="extracharge">' +
        '<td class="text-right" colspan="1"><strong>' +
        extrachargeText +
        '</strong></td>' +
        '<td  colspan="1" class="amount text-right nowrap">' +
        extrachargeReturnFormatted +
        ' ' +
        currency +
        '</td>' +
...
function p24setMethod(method) {
  $('form#przelewy24Form input[name=p24_method]').val(
    parseInt(method) > 0 ? parseInt(method) : '',
  )
  if (method == 266) {
...
function formSend() {
  var actionForm = $('.p24-register-card-wrapper').attr(
    'data-action-payment-check',
  )
  $.ajax(actionForm, {
    method: 'POST',
...
$(document).ready(function () {
  ...
  if (0 !== $('#extrachargeReturn').length) {
    p24DisplayExtrachargeReturn(
      $('#extracharge_text').val(),
      $('#extrachargeReturnFormatted').val(),
      $('#currencySign').val(),
    )
  }
  if (0 !== $('#extracharge').length) {
    p24DisplayExtrachargeAdmin(
      $('#extracharge_text').val(),
      $('#extrachargeFormatted').val(),
      $('#currencySign').val(),
    )
  }

  $('#P24FormArea').on('click', function (event) {
  ...
  $('.bank-box').on('click', function () {
    ...
    if (isSelected) {
      ...
      if ($(this).attr('data-action')) {
        ...
        $('form input[name="p24_card_customer_id"]').val(
  ...
  $('.p24-more-stuff').on('click', function () {
    $(this).fadeOut(100, function () {
      $('.p24-less-stuff').fadeIn()
    })
    $('.pay-method-list-second').slideDown()
  })
  $('.p24-less-stuff').on('click', function () {
    $(this).fadeOut(100, function () {
  ...
  //oneClick
  $('.p24-payment-return-page input.p24-remember-my-card').on(
    'change',
    function () {
    ...
    var callback = function (mutation) {
      try {
        ...
        document.getElementById(
          'P24_registerCard',
        ).checked = document.getElementById('p24-remember-my-card').checked
      } catch (ex) {}
    }
...
function requestJsAjaxCard() {
  p24showLoader()
  var actionForm = $('.p24-register-card-wrapper').attr(
    'data-action-register-card-form',
  )
  $.ajax(actionForm, {
    method: 'POST',
    type: 'POST',
    ...
    success: function (response) {
      ...
      if (data.error) {
        $('#p24-register-card-form')
          .fadeIn(200)
          .html('<p class="p24-text-error">' + data.error + '</p>')
      } else {
        ...
        var p24FormContainer = $('<div></div>')
          ...
          .attr(
            ...
            $('.p24-register-card-wrapper').attr('data-successCallback'),
          )
          .attr(
            'data-failureCallback',
            $('.p24-register-card-wrapper').attr('data-failureCallback'),
          )
            ...
            window.setTimeout(function () {
              $('#P24FormContainer button').on('click', function () {
                if (P24_Card.validate()) {
                  $(this).after(
                    '<div class="loading" id="card-loader" style="display: none"></div>',
                  )
...
$(function () {
  ...
  $images.each(function (idx, elm) {
    ...
    var $newBox = $('<span>')
      .addClass('bank-logo')
      .addClass('bank-logo-' + executedRx[1])
