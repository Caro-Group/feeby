							...
							{else}
								...
								{if isset($slider.image) && !empty($slider.image)}
									<div class="h-4 flex-shrink-0 flex justify-center items-center my-auto">
										<picture>
											<source src="{$slider.image|replace:".jpg":".webp"|replace:".png":".webp"escape:'html':'UTF-8'}" type="image/webp">
											<source src="{$slider.image|escape:'html':'UTF-8'}" type="image/jpeg">
											<img loading="{if $smarty.foreach.sliders.first}{else}lazy{/if}" class="h-full w-auto max-h-[16px]" data-lazy data-src="{$slider.image|escape:'html':'UTF-8'}"
											width="69" height="13"
