			...
			{foreach from=$sliders item=slider name="sliders"}
				...
				<div class="p-5 bg-white flex flex-col justify-between h-2/6 phablet:h-1/5 ">
					...
					{if isset($slider.title) && !empty($slider.title)}
						<h2 class="font-light tablet:text-base  desktop-presta:text-2xl line-clamp-2 phablet:line-clamp-1 desktop-wide:line-clamp-2">{$slider.title|escape:'html':'UTF-8' nofilter}</h2>
					{/if}
