                    ...
                    <ul class="flex ">
                        {foreach from=$formAtts.links item=item}
                            {if $item.title && $item.link}
                                <li class="ml-6">
                                    <a class="text-gray-main hover:text-main transition" href="{$item.link}"
                                        target="{$item.target_type}">
                                        {if $item.link|strstr:'facebook'}
                                            <svg class="fill-current text-gray-main hover:text-main transition" width="13" height="20" viewBox="0 0 13 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path
                                        ...
                                        {if $item.link|strstr:'instagram'}
                                            <svg class="fill-current text-gray-main hover:text-main transition" width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    {else}
        ...
        {elseif isset($formAtts.accordion_type) && ($formAtts.accordion_type == 'accordion' || $formAtts.accordion_type == 'accordion_small_screen')}{* Case : full or accordion*}
            <div
                ...
                {if isset($formAtts.title) && !empty($formAtts.title)}
                    <div class="title clearfix bg-white tablet:bg-transparent clearfix flex h-14 items-center px-6 tablet:px-0 border-0 border-b-2 border-solid border-gray-default tablet:border-b-0"
                        ...
                        {foreach from=$formAtts.links item=item}
                            {if $item.title && $item.link}
                                <li
                                    ...
                                    <a href="{$item.link}" target="{$item.target_type}">{$item.title|escape:'html':'UTF-8'}</a>
                                </li>
                            {/if}
                        {/foreach}
                    </ul>
