...
{if $hookName == 'displayNav1'}
    ...
    {/if}
        <div {if isset($formAtts.id) && $formAtts.id} id="{$formAtts.id|escape:'html':'UTF-8'}"{/if}
            class="tablet:py-2 bg-gray-1000 {if isset($formAtts.bg_img) && $formAtts.bg_img && isset($formAtts.bg_config) && $formAtts.bg_config == "boxed" && $aplazyload}lazy {/if}{(isset($formAtts.class)) ? ($formAtts.class|replace:'ApRow':''|replace:'ApRow':'') : ''|escape:'html':'UTF-8'} {(isset($formAtts.animation) && $formAtts.animation != 'none') ? ' has-animation' : ''} {$formAtts.bg_class}{*contain link can not escape*}"
