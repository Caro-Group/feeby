...
<div class="ginstagram_w">
    <div class="wrap_g_ins gtemplateFul">
        ...
        {if $getCfig->template == 1 && $getCfig->tem_ful_number != ''}
            {$nb = $getCfig->tem_ful_number}
        {else}
            {$nb = 10}
        {/if}
        <div id="ginsposition__{$getCfig->id_gembedinstagram|escape:'htmlall':'UTF-8'}_{$position|escape:'htmlall':'UTF-8'}_{$hooks|escape:'htmlall':'UTF-8'}" class="{if $getCfig->icon_show == 1}ginsshowicon{/if} ginstagramproduct ginstemplate_{$getCfig->template|escape:'htmlall':'UTF-8'} ginsposition__{$getCfig->id_gembedinstagram|escape:'htmlall':'UTF-8'}_{$position|escape:'htmlall':'UTF-8'}_{$hooks|escape:'htmlall':'UTF-8'} autoload_{$getCfig->tem_ful_autoload|escape:'htmlall':'UTF-8'}" 
                    ...
                    data-convert-fulC="{100/$getCfig->tem_ful_column|escape:'htmlall':'UTF-8'}"
                    ...
                    data-screen-col="{$getCfig->screen_column_f|escape:'htmlall':'UTF-8'}"
...
<style type="text/css">
    #module-g_productinstagram-instagram #wrapper .container{
        max-width: 100%;
    }
    .ginstagram_w > .wrap_g_ins {
        max-width: 100%;
        margin: 0 !important;
        
    }
    {if $getCfig->tem_ful_limit > 0}
        .ginsposition__{$getCfig->id_gembedinstagram|escape:'html':'UTF-8'}_{$position|escape:'html':'UTF-8'}_{$hooks|escape:'html':'UTF-8'} .gitem_:nth-child(n+ {$getCfig->tem_ful_limit|escape:'htmlall':'UTF-8' + 1}) {
