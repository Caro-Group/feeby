	...
	<div class="swiper-wrapper">
		{if $sliders}
			{foreach from=$sliders item=slider name="sliders"}
				<div class="swiper-slide" data-pausetime="{$slider.data_delay|escape:'html':'UTF-8'}">
					...
					{if $slider.video.active}
						...
						<div data-leo_image="{$slider.thumbnail|escape:'html':'UTF-8'}" data-leo_type="video"
							...
							data-autoplay="{$slider.video.videoauto}" data-videoid="{$slider.video.videoid|escape:'html':'UTF-8'}">
					...
					{else}
						...
						<!-- SLIDE IMAGE BEGIN -->
						<a {if $slider.data_link != ''} href="{$slider.data_link|escape:'html':'UTF-8'}" {/if}
							{if $slider.data_target != ''} target="{$slider.data_target|escape:'html':'UTF-8'}" {/if}
							{if $slider.background_color != ''}
							style="background-color:{$slider.background_color|escape:'html':'UTF-8'};" {/if}>

							<picture>
								<source
									data-srcset="{$slider.main_image|replace:" ":"%20"|replace:".jpg":".webp"|replace:".png":".webp"|escape:'html':'UTF-8'}"
									type="image/webp">
								<source data-srcset="{$slider.main_image|replace:" ":"%20"|escape:'html':'UTF-8'}"
									type="image/jpeg">
								<img class="block swiper-lazy min-w-full desktop-wide:h-banner-final object-left h-auto"
									...
									<div class="tp-caption {if $layer.layer_link}data-link{/if}{if $layer.layer_class} {$layer.layer_class|escape:'html':'UTF-8'}{/if}"
										...
										{if $layer.layer_link}onclick="event.stopPropagation();window.open('{$layer.layer_link|escape:'html':'UTF-8'}','{$layer.layer_target|escape:'html':'UTF-8'}');"
											...
											{if $layer.layer_type == "video"}
												...
												{else}
													<iframe
														src="http://www.youtube.com/embed/{$layer.layer_video_id|escape:'html':'UTF-8'}?wmode=transparent"
														width="{$layer.layer_video_width|escape:'html':'UTF-8'}"
														height="{$layer.layer_video_height|escape:'html':'UTF-8'}"></iframe>
												{/if}
											{/if}
