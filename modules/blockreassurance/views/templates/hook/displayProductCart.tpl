...
<div class="flex flex-wrap">
    {foreach from=$blocks item=$block key=$key}
        <div class="flex items-center justify-center mx-auto tablet-wide:w-1/2 w-full" {if $block['type_link'] !== $LINK_TYPE_NONE && !empty($block['link'])} style="cursor:pointer;" onclick="window.open('{$block['link']}')"{/if}>
