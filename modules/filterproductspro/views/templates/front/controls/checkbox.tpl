                ...
                <li class="max-w-full">
                    <label for="option_{$option.id_option|intval}" class=" !flex {if $filter.id_filter == 23} option-svg flex-col {else} whitespace-nowrap border-2 border-solid border-gray-2000 hover:border-gray-3000 !p-[9px] tablet:!p-[13px] {/if} items-center w-full cursor-pointer !mb-0 {if $option.selected } active {/if} rounded-[5px] transition duration-200 ">
                        ...
                        {if $filter.type eq 5 and $option.color and ($option.color.color or $option.color.texture)}
                            ...
                            {/if}
                        {else}
                        <span class="text-base tablet:mt-1 block text-ellipsis overflow-hidden text-main-dark w-auto {if $filter.id_filter == 23 && ($option.id_option == 969 || $option.id_option == 970)} max-w-[72px] text-center {else} max-w-full {/if} font-body font-normal">{$option.name|escape:'htmlall':'UTF-8'}</span>
                        {/if}
