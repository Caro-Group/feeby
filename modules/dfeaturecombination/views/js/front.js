...
function displayFeature(id_product_attribute) {
  ...
  $('.dfc_feature_inited')
    ...
    .filter(function () {
      return !$(
        ".product-features .product-features .dfc_table dt:contains('" +
          $.trim($(this).text()) +
          "')",
      ).length
    })
    .each(function (ind, obj) {
      $($(obj).next().clone()).insertAfter(
        $($(obj).clone()).insertAfter(
          $('.product-features .product-features .dfc_table dd:last-child'),
        ),
      )
    })
}
...
function displayDescription(id_product_attribute) {
  ...
  if (combination_description.length) {
    ...
    $('[href="#description"]').parent().show().find('a').trigger('click')
  } else {
...
function displayName(id_product_attribute) {
  ...
  if (
    typeof names[id_product_attribute] != 'undefined' &&
    names[id_product_attribute]
  ) {
...
function displayDescriptionShort(id_product_attribute) {
  ...
  var combination_description_short = $(
    '.dfc_short_description_' + id_product_attribute,
  )
...
$(document).ready(function () {
  ...
  $('.dfc_description_short_base').after(
    $('.dfc_description_short_section').html(),
  )
