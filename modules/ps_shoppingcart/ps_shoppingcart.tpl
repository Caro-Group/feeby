  ...
  <div class="blockcart cart-preview {if $cart.products_count > 0}active{else}inactive{/if}" data-refresh-url="{$refresh_url}">
    <div class="header">
      ...
      {/if}
        ...
        <span class="-right-2 absolute aspect-square bg-main bottom-0 cart-products-count h-[15px] leading-none max-w-[15px] overflow-hidden pt-0.5 rounded-full text-[12px] text-center text-ellipsis text-white text-xs">{$cart.products_count}</span>
      {if $cart.products_count > 0}
        </a>
      {/if}
    </div>
