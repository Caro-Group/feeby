  ...
  <div class="blockcart cart-preview {if $cart.products_count > 0}active{else}inactive{/if}" data-refresh-url="{$refresh_url}">
    <div class="header">
      ...
      {/if}
        ...
        <span class="cart-products-count absolute bg-main bottom-0 leading-none max-w-[15px] overflow-hidden px-1 right-0 rounded-full text-[12px] text-center text-ellipsis text-white text-xs">{$cart.products_count}</span>
      {if $cart.products_count > 0}
        </a>
      {/if}
    </div>
