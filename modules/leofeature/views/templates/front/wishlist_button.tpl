<div class="wislish-button relative">
  <a
    ...
    rel="nofollow"
  >
    <span class="bottom-[2px] -right-1 absolute w-full ap-total ap-total-wishlist aspect-square bg-main bottom-0 h-[15px] leading-none max-w-[15px] overflow-hidden pt-0.5 rounded-full text-[12px] text-center text-ellipsis text-white text-xs empty:invisible peer z-30">{if $logged}{WishList::getAllProductsFromAllWshlist($customer.id)}{/if}</span>
    <svg class="z-10 fill-current text-main-dark hover:text-main transition invisible peer-[:empty]:visible" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none">
    ...
    <svg class="z-20 absolute left-0 fill-current text-main-dark hover:text-main transition visible peer-[:empty]:invisible" xmlns="http://www.w3.org/2000/svg" width="29" height="29" fill="none">
