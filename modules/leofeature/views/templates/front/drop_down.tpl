			...
			<ul class="leo-dropdown-list-item overflow-auto">{foreach from=$cart.products item=product name="cart_product"}
					<li
						...
						<div class="leo-cart-item-img">
							{if $product.images}
								<a class="label" href="{$product.url}" title="{$product.name}"><img
										class="img-fluid border border-solid bg-gray-1000 rounded-md"
										src="{$product.images.0.bySize.small_default.url}" alt="{$product.name}"
										title="{$product.name}" /></a>
							{/if}
						</div>
						<div class="leo-cart-item-info">
							<div class="product-name"><a class="label font-normal" href="{$product.url}"
									title="{$product.name}">{$product.name|truncate:24:'...':true}</a></div>
						...
						{if ($product.attributes|count && $show_combination) || ($product.customizations|count && $show_customization)}
							<div class="view-additional">
								<div class="view-leo-dropdown-additional rounded-md bg-gray-1000 transition">
									<i
										class="material-icons -right-[3px] absolute select-none text-3xl text-main-dark transition transform duration-300 rotate-0 -translate-y-1"></i>
								</div>
							</div>
						{/if}
			...
			{if $freeshipping_price}
				...
				{if $remaining_to_spend > 0}
					<div class="leo_free_price">
						...
						<p class="font-body">{l s='Spent' d='Modules.Leofeature.Shop'}
