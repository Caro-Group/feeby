...
$(document).ready(function () {
  ...
  if (typeof show_popup != "undefined" && !show_popup) {
    $(".blockcart.cart-preview")
      .addClass("leo-blockcart")
      .removeClass("blockcart");
  }
  ...
  prestashop.on("updateCart", function (event) {
    ...
    if (event.reason.linkAction == "add-to-cart" && event.resp.success) {
      ...
      //DONGND:: run fly cart
      if (
        typeof enable_flycart_effect != "undefined" &&
        enable_flycart_effect
      ) {
        if ($(".leo-bt-cart.active").length) {
          ...
          $(".product-add-to-cart .add-to-cart").length
        ) {
    ...
    if ($(".leo-dropdown-cart-item.deleting").length) {
      ...
      $(
        ".leo-dropdown-cart-item.deleting .leo-dropdown-cssload-speeding-wheel"
      ).hide();
    ...
    if ($(".leo-dropdown-cart-item.updating").length) {
      ...
      $(
        ".leo-dropdown-cart-item.updating .leo-dropdown-cssload-speeding-wheel"
      ).hide();
      $(".leo-dropdown-cart-item.updating").removeClass("updating");
      showLeoNotification("success", "update", false);
    }
...
function leoBtCart() {
      ...
      $(this).on("click", function (event) {
        ...
        if (object_button_container.find(".leo_cart_quantity").length) {
          object_button_container
            .find(".qty_product")
            .val(object_button_container.find(".leo_cart_quantity").val());
        }

        var qty_product = object_button_container.find(".qty_product").val();
        var min_qty = object_button_container.find(".minimal_quantity").val();
        var quantity_product = object_button_container
          .find(".quantity_product")
          .val();
        // console.log(qty_product);
        // console.log(min_qty);
        // console.log(quantity_product);
        if (
          Math.floor(qty_product) == qty_product &&
          $.isNumeric(qty_product) &&
          qty_product > 0
        ) {
          // return true;
        } else {
        ...
        var id_product_attribute = object_button_container
          .find(".id_product_attribute")
          .val();
        var id_customization = object_button_container
          .find(".id_customization")
          .val();
            ...
            if (!$element.data("check-outstock")) {
              ...
              $.ajax({
                ...
                success: function (result) {
                  if (result.success) {
                    if (
                      $(".leo-notification").length &&
                      typeof enable_notification != "undefined" &&
                      enable_notification
                    ) {
                      //DONGND:: show notification
                    ...
                    //DONGND:: loading fly cart
                    if (
                      $(".leo-fly-cart .leo-fly-cart-cssload-loader").length
                    ) {
                      $(".leo-fly-cart .leo-fly-cart-cssload-loader").show();
                    }
                    ...
                    //DONGND:: loading default cart
                    if (
                      $(".leo-blockcart.cart-preview .cssload-piano").length
                    ) {
                      $(".leo-blockcart.cart-preview .cssload-piano").show();
                    }
                        ...
                        success: function (resp) {
                          if ($(".leo-bt-cart.active").length) {
                            ...
                            $(".leo-bt-cart.active")
                              .find(".leo-loading")
                              .hide();
                            $(".leo-bt-cart.active").removeClass(
                              "active reset"
                            );
                          }
  ...
  $(".leo_cart_quantity").each(function () {
    ...
    if ($(this).parents(".product-miniature").find(".qty_product").val()) {
      $(this).val(
        $(this).parents(".product-miniature").find(".qty_product").val()
      );
    } else {
      $(this).hide();
    }
...
function leoSelectAttr() {
  $(".leo-select-attr").on("click", function (e) {
    ...
    if (!$(this).hasClass("selected")) {
      ...
      $.ajax({
        ...
        url:
          ...
          "modules/leofeature/psajax.php?rand=" +
          new Date().getTime(),
        async: true,
        cache: false,
        data: {
        ...
        success: function (result) {
          if (result != "") {
            ...
            $product_article_e
              ...
              .attr("href", obj.product_url);
            $product_article_e
              .find(".product-price-and-shipping")
              .empty()
              .append(obj.price_attribute);
    ...
    parent_e
      .find(".qty_product_" + id_product)
      .val(min_qty_attr)
      .data("min", min_qty_attr);
...
function activeEventModal() {
  $(".leo-modal-cart").on("hide.bs.modal", function (e) {
    // console.log('test');
    $(".leo-modal-cart .modal-header").removeClass(
      "block-mess info-mess warning-mess"
    );
    $(".leo-modal-cart .modal-title").hide();
    var min_qty = $(".leo-bt-cart.reset")
      .parents(".button-container")
      .find(".minimal_quantity")
      .val();
    $(".leo-bt-cart.reset")
      .parents(".button-container")
      .find(".qty_product")
...
function updatePostionLabel($parent) {
  ...
  if ($percent.length) {
    ...
    $percent.css(
      "top",
      -$parent.find(".thumbnail-container").height() +
        $parent.find(".product-description").height() +
        FLAG_MARGIN
    );
  }
  if ($onsale.length) {
    $percent.css(
      "top",
      parseFloat($percent.css("top")) + $onsale.height() + FLAG_MARGIN
    );
...
function showDropDownCart($element, $type) {
  ...
  if ($type == "flycart") {
    object_element = $element
      .parents(".leo-fly-cart")
      .find(".leo-dropdown-cart");
  }
    ...
    if (!$element.hasClass("active-slidebarcart")) {
      ...
      if (pusheffect) {
        ...
        $("body.leoflycart-active-push main").css({
          ...
          "-webkit-transform":
            "translate" + push_type + "(" + push_value + "px)",
  ...
  $(".view-leo-dropdown-additional").on("click", function () {
    ...
    if (!$(this).hasClass("show")) {
      if (wrapper_parent_obj.find(".leo-dropdown-cart-item.show-additional")) {
        wrapper_parent_obj
          .find(".leo-dropdown-cart-item.show-additional")
          .removeClass("show-additional");
        wrapper_parent_obj
          .find(".view-leo-dropdown-additional.show")
          .removeClass("show");
      ...
      if (
        wrapper_parent_obj.hasClass("active-scrollbar") &&
        ...
        // else
        // {
        var height_clone_obj = parent_obj
          .find(".leo-dropdown-additional")
          .height();
        wrapper_parent_obj
          .find(".mCSB_container")
          .append(
            '<p class="fake-element" style="height:' +
              height_clone_obj +
              'px"></p>'
          );
        wrapper_parent_obj.mCustomScrollbar("update");
        ...
        success: function (result) {
          ...
          if (obj.success) {
            parent_obj.fadeOut(function () {
              ...
              parent_obj.remove();
              $(
                '.leo-remove-from-cart[data-id-product="' +
                  id_product +
                  '"][data-id-product-attribute="' +
                  id_product_attribute +
                  '"][data-id-customization="' +
                ...
                .parents(".leo-dropdown-cart-item")
                .remove();
              updateClassCartItem();
            });
            //DONGND:: show notification
  ...
  $(".leo-bt-product-quantity-down, .leo-bt-product-quantity-up").on(
    ...
    function () {
      ...
      var input_target = $(this)
        .parents(".leo-dropdown-cart-item")
        .find(".leo-input-product-quantity");
...
function updateQuantityProductDropDown($element) {
  ...
  if (
    Math.floor(input_quantity) == input_quantity &&
    $.isNumeric(input_quantity) &&
    input_quantity > 0
  ) {
    // return true;
  } else {
        ...
        $.ajax({
          ...
          success: function (result) {
            ...
            if (obj.success) {
              ...
              $(
                '.leo-input-product-quantity[data-id-product="' +
                  id_product +
                  '"][data-id-product-attribute="' +
                  id_product_attribute +
                  '"][data-id-customization="' +
                ...
                .val(input_quantity)
                .data("product-quantity", input_quantity);
              //DONGND:: show notification
              showLeoNotification("success", "update", false);
  ...
  if (typeof enable_dropdown_defaultcart != "undefined") {
    ...
    if (enable_dropdown_defaultcart) {
      if ($(".blockcart.cart-preview").length) {
        $(".blockcart.cart-preview")
          .addClass("leo-blockcart show-leo-loading")
          .append(
      ...
      $(".leo-blockcart.cart-preview.show-leo-loading").data(
        "type",
      ...
      if (result != "") {
        ...
        if (result.dropdown != "") {
          ...
          if (!$(".leo-dropdown-cart").length) {
            if (
              typeof type_dropdown_defaultcart != "undefined" &&
              (type_dropdown_defaultcart == "dropdown" ||
                type_dropdown_defaultcart == "dropup")
            ) {
              $(".leo-blockcart.cart-preview.show-leo-loading").after(
                '<div class="leo-dropdown-cart defaultcart ' +
                  type_dropdown_defaultcart +
                  '"></div>'
              );
            }
            ...
            if ($(".leo-fly-cart.enable-dropdown").length) {
              $(".leo-fly-cart.enable-dropdown").append(
                '<div class="leo-dropdown-cart flycart ' +
                  $(".leo-fly-cart.enable-dropdown").data("type") +
                  '"></div>'
              );
            }
            ...
            if ($(".leo-fly-cart-slidebar").length) {
              $(".leo-fly-cart-slidebar").append(
                '<div class="leo-dropdown-cart"></div>'
              );
            }
          } else {
          ...
          if ($(".leo-dropdown-cart-content").length) {
            ...
            if ($only_total == 1) {
              $(".leo-dropdown-cart-content .leo-dropdown-total").replaceWith(
                result.dropdown
              );
              var check_number_cartitem = 3;
              if (typeof number_cartitem_display != "undefined") {
              ...
              $(".leo-dropdown-list-item").each(function () {
                ...
                //DONGND:: scroll bar for slidebar
                if (
                  $(this).parents(".leo-fly-cart-slidebar").length &&
                  $(this)
                    .parents(".leo-fly-cart-slidebar")
                    .find(".active-scrollbar")
                ) {
                  // console.log('test');
                  checkFlyCartScrollBar($(this));
                }
        ...
        if ($(".leo-fly-cart-total").length) {
          if ($(".leo-dropdown-total").length) {
            $(".leo-fly-cart-total").text(
              $(".leo-dropdown-total").data("cart-total")
            );
          } else {
...
function flyCartEffect($element) {
  ...
  if (typeof product_img != "undefined" && product_img.length) {
    ...
    var gotoX =
      obj_element.offset().left +
      obj_element.width() / 2 -
      product_img.width() / divider / 2;
    //DONGND:: update for body boxed
    if ($("body").offset().left != 0) {
      gotoX = gotoX - $("body").offset().left;
    }
    if (
      $("html").hasClass("safari-win") &&
      $(".leo-fly-cart-slidebar.active .leo-fly-cart-icon").length
    ) {
      //DONGND:: update position of fly cart icon on slidebar cart for safari win
      var gotoY =
        $(window).scrollTop() +
        obj_element.offset().top +
        obj_element.height() / 2 -
        product_img.height() / divider / 2;
    } else {
      var gotoY =
        obj_element.offset().top +
        obj_element.height() / 2 -
...
function activeEventFlyCartSlideBar() {
  $(
    ...
    $(".leo-blockcart.cart-preview.active-slidebarcart").removeClass(
      "active-slidebarcart"
    );
    $("body.leoflycart-active-push main").css({
    ...
    setTimeout(function () {
      $("body").removeClass(
        "leoflycart-active-slidebar leoflycart-active-push"
      );
    }, 300);
...
function checkFlyCartScrollBar($element) {
  ...
  if (
    object_parent.hasClass("slidebar_top") ||
    object_parent.hasClass("slidebar_bottom")
  ) {
    // console.log('test');
    ...
    if (typeof width_cart_item != "undefined") {
      element_width =
        $element.find(".leo-dropdown-cart-item").length * width_cart_item;
    } else {
      element_width =
        $element.find(".leo-dropdown-cart-item").length *
        $element.find(".leo-dropdown-cart-item").outerWidth();
    }
  ...
  if (
    object_parent.hasClass("slidebar_left") ||
    object_parent.hasClass("slidebar_right")
  ) {
    var height_bottom = object_parent
      .find(".leo-dropdown-bottom")
      .outerHeight();
    var window_height = $(window).height();
    var element_height = "";
    if (typeof height_cart_item != "undefined") {
      element_height =
        $element.find(".leo-dropdown-cart-item").length * height_cart_item;
    } else {
      element_height =
        $element.find(".leo-dropdown-cart-item").length *
...
function checkFlyCartScrollBarDropDown($element) {
  var object_parent = $element.parents(
    ".leo-fly-cart.type-fixed.enable-dropdown"
  );
  var type = object_parent.data("type");
  var height_bottom = object_parent.find(".leo-dropdown-bottom").outerHeight();

  var height_real =
    height_cart_item * $element.find(".leo-dropdown-cart-item").length;
  var height_icon = object_parent
    .find(".leo-fly-cart-icon-wrapper")
    .outerHeight();
  var window_height = $(window).height();
  var check_number_cartitem = 3;
  if (typeof number_cartitem_display != "undefined") {
  ...
  if (
    ...
    type == "dropup" &&
    height_real + height_bottom > object_parent.position().top &&
    ...
    object_parent.length > 0 &&
    type == "dropdown" &&
    height_real + height_bottom >
      $(window).height() - object_parent.position().top &&
    height_default + height_bottom >
      $(window).height() - object_parent.position().top
  ) {
    // console.log('test1');
    $element.addClass("active-scrollbar").css({
...
function getOffsetFlycartIcon() {
  if ($(".leo-fly-cart.solo .leo-fly-cart-icon").length) {
    ...
    if (offset_left <= window_width / 2) {
      $(".leo-fly-cart.solo")
        .removeClass("offset-right")
        .addClass("offset-left");
    } else {
      $(".leo-fly-cart.solo")
        .removeClass("offset-left")
        .addClass("offset-right");
    }
...
function leoChooseAttr() {
  $(document).on(
    "change",
    ".variants-product [data-product-attribute]",
    function (e) {
      var id_product = parseInt(
        $(this).parents(".addToCartFormWrapper:eq(0)").attr("data-product-id")
      );
      var group_value = $(this)
        .parents(".addToCartFormWrapper:eq(0)")
        .find(".addToCartForm")
        .serialize();
      var parent_e = $(this).parents(".product-miniature");
      ...
      $.ajax({
        ...
        url:
          "http://localhost/prestashop_17/ps_1765_dev/module/leofeature/leoproductattribute?ajax=1&action=getproductattribute&" +
          group_value,
        ...
        success: function (result) {
          if (result != "") {
            ...
            parent_e
              .find(".id_product_attribute_" + id_product)
              .val(obj.id_attr);
            if (!obj.add_to_cart_url) {
              parent_e.find(".leo-bt-cart_" + id_product).addClass("disabled");
            } else {
              parent_e
                .find(".leo-bt-cart_" + id_product)
